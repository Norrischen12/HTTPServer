...
static void handle_file(const string& fpath, WordIndex *index) {
  ...
  boost::trim(content);
  boost::replace_all(content, "A", "a");
  boost::replace_all(content, "B", "b");
  boost::replace_all(content, "C", "c");
  boost::replace_all(content, "D", "d");
  boost::replace_all(content, "E", "e");
  boost::replace_all(content, "F", "f");
  boost::replace_all(content, "G", "g");
  boost::replace_all(content, "H", "h");
  boost::replace_all(content, "I", "i");
  boost::replace_all(content, "J", "j");
  boost::replace_all(content, "K", "k");
  boost::replace_all(content, "L", "l");
  boost::replace_all(content, "M", "m");
  boost::replace_all(content, "N", "n");
  boost::replace_all(content, "O", "o");
  boost::replace_all(content, "P", "p");
  boost::replace_all(content, "Q", "q");
  boost::replace_all(content, "R", "r");
  boost::replace_all(content, "S", "s");
  boost::replace_all(content, "T", "t");
  boost::replace_all(content, "U", "u");
  boost::replace_all(content, "V", "v");
  boost::replace_all(content, "W", "w");
  boost::replace_all(content, "X", "x");
  boost::replace_all(content, "Y", "y");
  boost::replace_all(content, "Z", "z");
  boost::split(words, content, [](char c) { return !std::isalpha(c); });  
  
  for (auto i : words) {
    if (i != ""){
      index -> record(i, fpath);
    }
