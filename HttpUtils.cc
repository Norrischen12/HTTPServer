...
bool is_path_safe(const string &root_dir, const string &test_file) {
  ...
  // realpath(): to get the absolute paths of the root directory and test file
  if (realpath(root_dir.c_str(), abs_root) == nullptr || 
      realpath(test_file.c_str(), abs_test) == nullptr) {
      return false;
  }
  strcat(abs_root, "/");
  // Make sure the test file is a subdirectory of the root directory
  return !strncmp(abs_root, abs_test, strlen(abs_root));
}
